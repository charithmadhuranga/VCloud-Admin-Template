...
export default function Dropdown({ isOpen, onClose, children, className = '', align = 'right' }) {
  ...
  return (
    <div
      ref={ref}
      className={`absolute z-50 mt-2 min-w-[220px] rounded-xl border border-(--color-border) bg-(--color-surface-elevated) shadow-lg animate-fade-in ${
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors cursor-pointer ${
        danger ? 'text-grafana-red hover:bg-grafana-red/5' : 'text-text-secondary hover:bg-(--color-surface-hover) hover:text-text-primary'
      } ${className}`}
    >
      {Icon && <Icon size={15} />}
      {children}
