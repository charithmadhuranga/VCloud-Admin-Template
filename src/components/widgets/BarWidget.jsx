...
export default function BarWidget({ widget, data, loading }) {
  ...
  const categories = data.map((d) => String(d[categoryColumn] ?? ""));
  const seriesData = data.map((d) => Number(d[valueColumn] ?? 0));
    ...
    <Card className="p-3 h-full flex flex-col">
      ...
      {loading ? (
        ...
        <div className="flex-1 min-h-0">
          <BarChart
            data={[{ name: valueColumn, data: seriesData }]}
